...
static class Predictor
{
    ...
    private static void DrawVector(Bitmap canvas, Point point, Point vector, Color color)
    {
        ...
        VecPoint vecPoint = new VecPoint(vector);
        Vector2 v2Start = new Vector2(point.X, point.Y);
        ...
        while (traveledDistance * traveledDistance < vecPoint.LengthSquared)
        {
            ...
            if (DataSource.mapBounds.Contains(walkPoint))
            {
            ...
            walkPoint = new Point((int)drawPoint.X, (int)drawPoint.Y);
        ...
        for (int x = 0; x < 5; x++)
        {
            ...
            if (DataSource.mapBounds.Contains(drawPoint))
